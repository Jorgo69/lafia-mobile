...
<div class="space-y-5">
    ...
    @if (!$identity)
        <div class="text-center py-12 space-y-4">
            <div class="w-16 h-16 mx-auto rounded-full bg-indigo-50 dark:bg-indigo-900/30 flex items-center justify-center">
                <x-icon name="shield-check" class="w-8 h-8 text-indigo-500" />
            </div>
        ...
        <section>
            <div class="flex items-center justify-between mb-3">
                ...
                @if(count($guardians) < $identity['threshold'] + 1)
                    <button wire:click="$toggle('showAddGuardian')" class="text-sm text-indigo-500 font-medium flex items-center gap-1.5 min-h-[44px] px-3 press-feedback rounded-xl">
            ...
            @else
                <x-card class="text-center py-6 space-y-3">
                    ...
                    @if (count($guardians) >= $identity['threshold'])
                        <button wire:click="$toggle('showRecovery')" class="text-sm text-amber-600 font-semibold min-h-[44px] px-4 press-feedback rounded-xl">
