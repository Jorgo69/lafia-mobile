        ...
        <form wire:submit="save" class="space-y-4">
            ...
            <x-card class="space-y-4">
                <div>
                    ...
                    <select wire:model="bloodType" class="w-full rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm py-3 min-h-11">
                        <option value="">—</option>
                        @foreach (\App\Modules\Vault\Enums\BloodType::cases() as $bt)
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="number" wire:model="weightKg" step="0.1" class="w-full rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm py-3 min-h-11" placeholder="72.5">
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-1">{{ __('vault.height') }}</label>
                        <input type="number" wire:model="heightCm" step="0.1" class="w-full rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm py-3 min-h-11" placeholder="175">
                    </div>
                </div>
            </x-card>
            ...
            <x-card>
                ...
                <textarea wire:model="emergencyNotes" rows="3" class="w-full rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm py-3 px-4 min-h-11"></textarea>
            </x-card>
    ...
    @elseif ($decryptedData)
        <x-card class="space-y-3">
            <div class="flex items-center justify-between">
                ...
                <button wire:click="edit" class="text-sm text-primary-500 font-medium flex items-center gap-1 min-h-11 px-2 press-feedback rounded-xl">
                    <x-icon name="pencil" class="w-3.5 h-3.5" />
                </button>
            </div>

            @if ($decryptedData['blood_type'])
                <div class="flex items-center gap-3 p-3 bg-danger-50 dark:bg-danger-900/20 rounded-2xl">
                    <x-icon name="droplets" class="w-8 h-8 text-danger-500" />
                    <div>
                        <div class="text-2xs uppercase text-gray-400 font-bold">{{ __('vault.blood_type') }}</div>
                        <div class="text-xl font-black text-danger-500">{{ $decryptedData['blood_type'] }}</div>
                    </div>
                </div>
            @endif

            @if (count($decryptedData['allergies']) > 0)
                <div class="p-3 bg-warning-50 dark:bg-warning-900/20 rounded-2xl">
                    <div class="text-2xs uppercase text-gray-400 font-bold mb-1.5">{{ __('vault.allergies') }}</div>
            ...
            @if (count($decryptedData['medications']) > 0)
                <div class="p-3 bg-primary-50 dark:bg-primary-900/20 rounded-2xl">
                    <div class="text-2xs uppercase text-gray-400 font-bold mb-1.5">{{ __('vault.medications') }}</div>
                    <div class="flex flex-wrap gap-1.5">
            ...
            @if (count($decryptedData['conditions']) > 0)
                <div class="p-3 bg-success-50 dark:bg-success-900/20 rounded-2xl">
                    <div class="text-2xs uppercase text-gray-400 font-bold mb-1.5">{{ __('vault.conditions') }}</div>
                    <div class="flex flex-wrap gap-1.5">
            ...
            @if ($decryptedData['emergency_notes'])
                <div class="p-3 bg-gray-50 dark:bg-gray-800 rounded-2xl">
                    <div class="text-2xs uppercase text-gray-400 font-bold mb-1">{{ __('vault.emergency_notes') }}</div>
