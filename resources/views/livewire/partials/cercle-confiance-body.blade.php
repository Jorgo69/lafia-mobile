@if (!$identity)
    <div class="text-center py-12 space-y-4">
        <div class="w-16 h-16 mx-auto rounded-full bg-secondary-50 dark:bg-secondary-900/30 flex items-center justify-center">
            <x-icon name="shield-check" class="w-8 h-8 text-secondary-500" />
        </div>
        <h2 class="text-base font-bold">{{ __('circle.title') }}</h2>
        <p class="text-sm text-gray-500 max-w-xs mx-auto">{{ __('circle.recovery_info') }}</p>
        <x-btn variant="primary" wire:click="registerDevice" loading="registerDevice" class="min-w-[180px]">
            <span class="flex flex-col items-center gap-1">
                <x-icon name="smartphone" class="w-6 h-6 shrink-0" />
                <span>{{ __('circle.register_device') }}</span>
            </span>
        </x-btn>
    </div>
...
@else
    ...
    <x-card>
        <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-full bg-success-100 dark:bg-success-900/30 flex items-center justify-center shrink-0">
                <x-icon name="smartphone" class="w-5 h-5 text-success-600" />
            </div>
            <div class="flex-1 min-w-0">
                <div class="font-semibold text-sm truncate">{{ $identity['device_name'] ?? __('circle.my_device') }}</div>
                <div class="text-2xs text-gray-400 font-mono">{{ substr($identity['fingerprint'], 0, 16) }}...</div>
            </div>
            @php $deviceStatus = \App\Modules\Identity\Enums\DeviceStatus::tryFrom($identity['status']); @endphp
            <span class="text-2xs font-bold px-2 py-1 rounded-full {{ $deviceStatus?->badgeClass() ?? '' }}">
    ...
    <section>
        <div class="flex items-center justify-between mb-3">
            ...
            @if(count($guardians) < $identity['threshold'] + 1)
                <button wire:click="$toggle('showAddGuardian')" class="text-sm text-secondary-500 font-medium flex items-center gap-1.5 min-h-11 px-3 press-feedback rounded-xl">
    ...
    <section>
        ...
        @if ($activeRecovery)
            <x-card class="border-warning-300 border-2 space-y-3">
                <div class="flex items-center gap-2">
                    <x-icon name="timer" class="w-5 h-5 text-warning-500" />
                    <span class="font-semibold text-sm text-warning-600">{{ __('circle.recovery') }}</span>
                </div>
                <div class="flex items-center gap-4">
                    <div class="flex-1 bg-gray-100 dark:bg-gray-700 rounded-full h-3 overflow-hidden">
                        <div class="bg-warning-500 h-full rounded-full transition-all"
        ...
        @else
            <x-card class="text-center py-6 space-y-3">
                ...
                @if (count($guardians) >= $identity['threshold'])
                    <button wire:click="$toggle('showRecovery')" class="text-sm text-warning-600 font-semibold min-h-11 px-4 press-feedback rounded-xl">
