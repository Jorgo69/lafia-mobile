...
>
    ...
    <div x-show="checking" class="text-center">
        <div class="w-16 h-16 rounded-full bg-indigo-100 dark:bg-indigo-900/30 flex items-center justify-center mx-auto mb-4">
            <x-icon name="lock" class="w-8 h-8 text-indigo-500 animate-pulse" />
        </div>
    ...
    <div x-show="!checking && failed" x-cloak class="text-center">
        <div class="w-16 h-16 rounded-full bg-red-100 dark:bg-red-900/30 flex items-center justify-center mx-auto mb-4">
            <x-icon name="lock" class="w-8 h-8 text-red-500" />
        </div>
        <p class="text-sm text-gray-600 dark:text-gray-400 mb-6">{{ __('vault.auth_failed') }}</p>

        <button
            @click="promptAuth()"
            class="bg-indigo-500 text-white px-8 py-3 rounded-2xl font-semibold press-feedback mb-3"
        >
            {{ __('vault.auth_retry') }}
        </button>
    </div>
