    ...
    <div class="flex items-stretch max-w-md mx-auto">
        @foreach ($tabs as $tab)
            ...
            <a href="{{ $tab['href'] }}" wire:navigate
               class="flex-1 flex flex-col items-center justify-center gap-0.5 py-2.5 min-h-[56px] press-feedback
                ...
                <span class="text-[11px] {{ $active ? 'font-bold' : 'font-semibold' }} leading-tight">{{ $tab['label'] }}</span>
            </a>
        @endforeach
    </div>
</nav>
